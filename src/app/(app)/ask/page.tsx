      ...
      <Card className="flex flex-col h-[calc(100vh-8rem)]">
        <CardHeader>
            <div className="flex items-center gap-2">
                ...
                <h2 className="text-2xl font-bold tracking-tight font-headline">Ask AI Tutor</h2>
            </div>
            <p className="text-muted-foreground">Get instant help with your STEM questions in your native language.</p>
        </CardHeader>
            ...
            <div className="space-y-6 pr-4">
              {conversation.length === 0 ? (
                ...
                conversation.map((entry, index) => (
                  <div key={index} className={cn("flex items-start gap-4", entry.role === 'user' && 'justify-end')}>
                    {entry.role === 'assistant' && (
                        <Avatar>
                            <AvatarFallback><Bot /></AvatarFallback>
                        </Avatar>
                    )}
                    <div className={cn("max-w-[75%] rounded-lg p-3", entry.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted')}>
        ...
        <CardFooter className="pt-4 border-t">
           <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full items-start gap-4">
              <FormField
